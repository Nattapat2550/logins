...
const { requireAuth } = require('../middleware/auth'); // For logout

const router = express.Router();

// Passport setup for Google OAuth (continued from previous)
passport.use(new GoogleStrategy({
  ...
  try {
    ...
    if (!user) {
      ...
      user.is_email_verified = true; // Assumed verified via Google
    }
    done(null, user);
  } catch (err) {
    done(err);
...
// Rate limits (per spec: IP-based, custom messages)
...
const verifyLimiter = rateLimit({
  ...
  max: 5, // 5 per user (use email as key)
...
function setAuthCookie(res, user, remember = false) {
  ...
  const exp = remember ? 30 * 24 * 60 * 60 : 7 * 24 * 60 * 60; // seconds
  const token = jwt.sign(payload, process.env.JWT_SECRET, { expiresIn: exp });
  const maxAge = exp * 1000; // ms
...
router.post('/complete-profile', async (req, res) => {
  try {
    ...
    if (!email || !username || (!password && !req.query.google)) { // Password optional for Google
...
router.get('/google/callback',
  ...
  (req, res) => {
    ...
    const remember = req.query.remember === 'true'; // Optional param
    ...
    if (!req.user.username || !req.user.password_hash) {
      // Incomplete profile: redirect to form
...
router.post('/forgot-password', resetLimiter, async (req, res) => {
  try {
    ...
    if (!user) {
      ...
      return res.json({ success: true, message: 'RESET_SENT', data: { email } });
    }
...
router.post('/reset-password', async (req, res) => {
  try {
    ...
    await userModel.setPassword(userId, password);
