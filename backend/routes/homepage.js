...
const { pool } = require('../config/db');  // Added: Import database pool
...
router.get('/', authenticateToken, async (req, res) => {
    try {
        ...
        console.error('Homepage GET error:', err.message || err);  // Better logging: Include message
...
router.put('/', authenticateToken, isAdmin, async (req, res) => {
    ...
    try {
        ...
        const result = await pool.query(
            'INSERT INTO homepage (id, content_text, content_image) VALUES (1, $1, $2) ON CONFLICT (id) DO UPDATE SET content_text = $1, content_image = $2 RETURNING *',
            [content_text || 'Default homepage content.', content_image || '']
        );
        console.log('Homepage content updated successfully');  // Debug
        res.json(result.rows[0]);
    } catch (err) {
        console.error('Homepage PUT error:', err.message || err);
