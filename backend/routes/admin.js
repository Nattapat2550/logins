...
router.put('/users/:id', authenticateJWT, isAdmin, async (req, res) => {
  try {
    ...
    if (e && e.status === 409) return res.status(409).json({ error: 'Duplicate value' });
...
router.put('/carousel/:id', authenticateJWT, isAdmin, upload.single('image'), async (req, res) => {
  try {
    ...
    const token = extractToken(req);
    const updated = await updateCarouselItem(
      id,
      {
        ...
        image_dataurl: dataUrl !== undefined ? dataUrl : undefined,
      },
      token
    );
